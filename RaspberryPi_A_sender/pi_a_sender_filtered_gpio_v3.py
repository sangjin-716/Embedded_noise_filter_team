# 버튼 중복 입력 방지
# 초음파 사람 감지되면 즉시 on, 센서가 감지하지 못해도 감지 시점부터 10초 동안은 계속 on
# 최종본 
...
SAMPLE_RATE = 48000
CHANNELS = 1
CHUNK = 3840          # 10ms @ 48kHz, RNNoise 프레임과 동일
DTYPE = "int16"
# =======================
...
ULTRA_INTERVAL = 1.0         # 거리 측정 주기(초) – 원래 네가 둔 값 유지
PERSON_HOLD_SEC = 10.0       # 마지막 감지 후 10초 동안은 ON 유지

# 버튼 디바운스 시간
DEBOUNCE_SEC = 0.2           # 한 번 유효 입력 후 200ms 동안 모든 버튼 입력 무시
# ===============================
...
def button_poll_thread():
    """
    이벤트 인터럽트 대신, 10ms마다 버튼을 읽어서
    눌린 순간(High → Low 변 transition)만 MODE 변경.
    디바운싱: 한 번 유효 입력 후 DEBOUNCE_SEC 동안 모든 버튼 입력 무시.
    """
    global MODE, running
    ...
    while running:
        ...
        for idx, pin in enumerate(BTN_PINS):
            ...
            if last_state[idx] == 1 and cur == 0:
                # 직전에 누른 지 DEBOUNCE_SEC 이상 지났을 때만 인정
                if now - last_pressed_time >= DEBOUNCE_SEC:
                    MODE = idx  # 0~3
                    mix = MODE_RNN_MIX.get(MODE, 0.0)
                    print(
                        f"[BTN] Button {idx+1} (GPIO{pin}) 눌림 → "
                        f"MODE={MODE} ({MODE_NAME[MODE]}), RNN_MIX={mix}"
                    )
...
def ultrasonic_thread():
    """
    사람 감지 로직:
      - dist < PERSON_THRESHOLD_CM 이 나오면 즉시 person_present = True
        + last_detect_time 갱신
      - 그 이후로 감지가 안 되어도 마지막 감지 시점에서 PERSON_HOLD_SEC(10초)까지는 계속 True 유지
      - 마지막 감지 후 10초가 완전히 지나야 person_present = False
    """
    ...
    while running:
        ...
        if dist is not None and dist < PERSON_THRESHOLD_CM:
            # 사람 새로 감지됨 → 즉시 ON
            last_detect_time = now
            person_present = True
        else:
            # 최근 감지 이력이 있으면 홀드 시간 체크
            if last_detect_time is not None and (now - last_detect_time) < PERSON_HOLD_SEC:
                person_present = True
            else:
                person_present = False
